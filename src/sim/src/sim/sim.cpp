...
double
random_number( double posB , double negB ){
	return negB + ( 2.0 * posB * (double) (rand() % 1000) / (double) (1000));
}
...
sample( const double& bsquared ){
	double tmp = 0.0;
	//fill this in
	double b = pow(bsquared , 0.5);
	for(unsigned int i = 0; i < 12; i++){
		tmp += random_number(-b,b);
	}
...
step( const double& dt ){
	...
	//fill this in
	uhat(0) = _u(0) + ( ( _alpha1 * ( _u(0) * _u(0) ) ) + ( _alpha2 * ( _u(1) * _u(1) ) ) );
	uhat(1) = _u(1) + ( ( _alpha3 * ( _u(0) * _u(0) ) ) + ( _alpha4 * ( _u(1) * _u(1) ) ) );
	...
	if(uhat(1) != 0){
		...
		dx(0) = ( uhat(0) * cos( _x(2) ) );
		dx(1) = ( uhat(0) * sin( _x(2) ) );
		dx(2) = 0;
	}
