...
DriveToPoint::
DriveToPoint()
	: odometry(), path(), command(), projection(), lookahead(), max_speed( 0.5 ), path_index( 0 ), distance_threshold( 0.1 ), angle_threshold( M_PI/8.0 ) {
}
...
handlePath( const nav_msgs::Path::ConstPtr& msg ){
	path = *msg;
	path_index = 0;
	std::cout << "got path with " << path.poses.size() << " poses" << std::endl;
	return;
}
